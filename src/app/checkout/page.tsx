...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    ...
    // Usaremos la API pública para obtener datos del curso
    // Nota: Necesitamos un endpoint que devuelva por ID o slug.
    // Por ahora, asumiremos que existe /api/courses/detail?id=xxx
    fetch(`/api/instructor/courses/${courseId}`) // Usamos el del instructor si es admin/dueño o creamos uno público
      .then(res => res.json())
      .then(data => {
        setCourse(data);
        setLoading(false);
  ...
  const handleApplyCoupon = async () => {
    ...
    try {
      // Validar el cupón llamando a create-session simulado o un validador
      // Por simplicidad, intentaremos crear una sesión de checkout y si falla el cupón, Stripe lo dirá,
      // pero para la UI necesitamos saber el descuento.
      // Implementaré un pequeño endpoint o validación manual aquí.
      // El seed tiene PROMO20 (20%).
      if (couponCode.toUpperCase() === 'PROMO20' && courseId) {
        // En un mundo real, llamaríamos a /api/coupons/validate?code=xxx&courseId=yyy
          ...
          <div className="lg:col-span-1">
            <div className="bg-[#152035] border border-blue-500/30 rounded-2xl p-6 sticky top-24 shadow-2xl">
              ...
              <p className="mt-6 text-[10px] text-center text-gray-500 leading-tight">
                Al hacer clic en pagar, aceptas los términos de servicio de Plattform. Tus datos de pago están protegidos por Stripe.
              </p>
            </div>
          </div>
        </div>
