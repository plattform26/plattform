...
function LessonEditor({
  lesson, moduleId, courseId, onSave, onDelete, isLocked
}: {
  lesson: any; moduleId: string; courseId: string; onSave: (l: any) => void; onDelete: (id: string) => void; isLocked?: boolean;
}) {
  ...
  return (
    <div className="bg-[#0a1626] border border-blue-500/15 rounded-xl overflow-hidden">
      ...
      >
        ...
        >
          <span className="text-gray-500 text-[10px] group-hover:text-cyan-400 transition-colors uppercase font-bold">{open ? 'CERRAR' : 'ABRIR'}</span>
        </div>
        <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
          {saved && <span className="text-green-400 text-xs">✓</span>}
          {!isLocked && <button onClick={handleDelete} className="text-red-400/60 hover:text-red-400 transition-colors text-xs px-1">✕</button>}
        </div>
      </div>
...
function ModuleBlock({
  mod, courseId, onDelete, onLessonAdd, onLessonUpdate, onLessonDelete, onTitleChange, isLocked
}: any) {
    ...
    <div className="bg-[#0d1524] border border-blue-500/20 rounded-2xl overflow-hidden mb-4">
      ...
      >
        ...
        <div className="flex items-center gap-2 shrink-0" onClick={e => e.stopPropagation()}>
          ...
          {!isLocked && <button onClick={() => setEditTitle(true)} className="text-gray-500 hover:text-white text-xs px-2 py-1 transition-colors">✏️</button>}
          {!isLocked && <button onClick={() => onDelete(mod.id)} className="text-red-400/50 hover:text-red-400 text-xs px-2 py-1 transition-colors">✕</button>}
          <div 
...
export default function ModulesPage() {
  ...
  return (
    <>
      {isLocked && (
        <div className="fixed top-28 left-1/2 -translate-x-1/2 z-[1000] w-full max-w-2xl px-6">
          <div className="bg-blue-600 border border-cyan-400/50 rounded-2xl p-4 shadow-2xl flex items-center gap-4 animate-in slide-in-from-top duration-500">
            ...
            <div className="text-white text-left">
              <p className="font-bold text-sm">Contenido Protegido</p>
              <p className="text-xs opacity-90 leading-tight">Este curso tiene alumnos activos. No puedes modificar la estructura ni el contenido para garantizar la integridad de la experiencia del alumno.</p>
            </div>
            <Link href={`/dashboard/instructor/courses/${courseId}`} className="ml-auto px-4 py-2 bg-black/20 hover:bg-black/40 rounded-xl text-xs font-bold transition-all shrink-0">Volver</Link>
          </div>
        </div>
      )}

      <div className={`flex items-start justify-between mb-6 flex-wrap gap-4 ${isLocked ? 'opacity-80 pointer-events-none' : ''}`}>
        <div>
          <Link href={`/dashboard/instructor/courses/${courseId}`} className="text-gray-400 hover:text-white text-sm transition-colors flex items-center gap-2 mb-3">
            ← {course.title}
          </Link>
        ...
        <Link href={`/dashboard/instructor/courses/${courseId}/quiz`} className="px-4 py-2 bg-purple-500/10 border border-purple-500/20 hover:bg-purple-500/20 rounded-xl text-purple-400 text-xs font-bold transition-colors pointer-events-auto">
          Ir a Evaluación →
        </Link>
      </div>

      <div className={`space-y-0 ${isLocked ? 'pointer-events-none grayscale-[0.3]' : ''}`}>
        {modules.map(mod => (
          <ModuleBlock
            key={mod.id}
            mod={mod}
      ...
      {!isLocked && (
        ...
        <div className="mt-12 pt-10 border-t border-blue-500/10 flex justify-center">
           <button 
             ...
             disabled={isLocked}
             className={`px-10 py-4 rounded-2xl text-sm font-black transition-all uppercase tracking-[0.2em] ${
               isLocked 
               ? 'bg-gray-800 text-gray-600 cursor-not-allowed opacity-50' 
               : 'bg-gradient-to-r from-blue-600 to-cyan-500 hover:scale-[1.02] active:scale-[0.98] text-white shadow-xl shadow-blue-500/20'
             }`}
           >
              {isLocked ? '🔒 Editor Bloqueado' : 'Guardar cambios'}
           </button>
        </div>
