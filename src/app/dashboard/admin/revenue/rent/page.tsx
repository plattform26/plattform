...
export default function AdminRevenueRentPage() {
  ...
  const handleExportCSV = () => {
    ...
    exportToCSV(exportData, `plattform-rentas-${filters.month + 1}-${filters.year}`);
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(exportData, `plattform-rentas-${filters.month + 1}-${filters.year}`, 'Rentas');
  };
  ...
  return (
    <div className="space-y-10">
       <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div>
             <h1 className="text-3xl font-space-grotesk font-bold">Rentas por <span className="text-cyan-400">Instructor</span></h1>
             <p className="text-gray-400 mt-2 font-light tracking-wide italic">SaaS Economics: Control de recurrencia y cumplimiento de cuotas de alumnos.</p>
          </div>
       ...
       {/* KPIs RENTAS CON FILTRO TEMPORAL */}
       <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#0d1524] border border-blue-500/10 p-6 rounded-3xl shadow-2xl shadow-cyan-500/5 relative overflow-hidden group">
             <div className="absolute top-0 right-0 p-4 flex gap-2">
                <select 
                  value={filters.month} 
                  onChange={(e) => setFilters(prev => ({ ...prev, month: parseInt(e.target.value) }))}
                  className="bg-black border border-cyan-500/50 text-white rounded-lg text-[10px] px-2 py-1 focus:ring-1 focus:ring-cyan-400 outline-none transition-all cursor-pointer hover:bg-slate-900"
                >
             ...
             <p className="text-4xl font-bold text-white tracking-tighter italic animate-in fade-in slide-in-from-left duration-700" suppressHydrationWarning>
          ...
          <div className="bg-[#0d1524] border border-blue-500/10 p-6 rounded-3xl shadow-2xl shadow-blue-500/5">
             <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2">Proyección Anual {filters.year}</p>
             <p className="text-2xl font-bold text-cyan-400 tracking-tighter italic" suppressHydrationWarning>
               ${new Intl.NumberFormat('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(data.metrics.accumulatedYear)}
             </p>
             <p className="text-[9px] text-gray-600 mt-2 uppercase">Basado en transacciones confirmadas</p>
          </div>
          <div className="bg-red-500/5 border border-red-500/10 p-6 rounded-3xl shadow-2xl shadow-red-500/5">
             <p className="text-[10px] font-black text-red-400 uppercase tracking-widest mb-2">Suscripciones Críticas</p>
             <p className="text-2xl font-bold text-red-500 tracking-tighter italic">{data.metrics.expiredCount}</p>
             <p className="text-[9px] text-red-500/40 mt-2 uppercase tracking-tighter">Vencidas / Morosas / Canceladas</p>
          </div>
       </div>
          ...
          <table className="w-full text-left border-collapse">
             <thead className="bg-[#152035]/50 border-b border-blue-500/10">
                <tr>
                   ...
                   <th className="p-6 text-[10px] font-black uppercase tracking-[0.2em] text-gray-500">Monto</th>
