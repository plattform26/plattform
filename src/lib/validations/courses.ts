...
export const createLessonSchema = z.object({
  ...
  videoUrl: z.union([z.string().url('URL de video inválida'), z.literal('')]).optional().nullable(),
  contentType: z.enum(['TEXT', 'VIDEO', 'QUIZ']).default('TEXT'),
  orderIndex: z.number().int().min(0).optional(),
  durationMinutes: z.union([z.coerce.number().int().min(0), z.literal('')]).optional().nullable(),
